...
config.js          window.CONFIG。タイマー/ラン(runQuizzes/clearBonus/nextMs)/ヒント/矢印幾何/マーカー等の調整値。
...
## 7. ゲーム進行（コアループ）
- 1ラン＝登録クイズから**ランダム5問**（`pickRun()` / `config.runQuizzes`）。
- タイマーは**5問通しで1本**（`config.timerSeconds`）。1問クリアごとに `config.clearBonus`（=10秒）加算。クリア画面中は `paused` で停止。
- 問題間：クリア画面に **NEXT** ボタン → 押すとヒントと同じプログレスバーが `config.nextMs`（=1.5秒）走り、完了で `loadQuiz(次)`。
- 時間切れ＝**ラン全体ゲームオーバー**（`gameOver` → リトライ/タイトル）。
- 5問クリア＝**ALL CLEAR**（`#overAllClear`：クリア5問の絵を一様表示・**レイアウトは仮**）→ タップで `location.reload()`＝タイトル。
- ヒントは1問ごとに `config.hintCount` にリセット。
- `loadQuiz(i)`：その問題の words / `HINT_NAME` / `WIN_SRC` / 盤面 / 状態をリセット（タイマーは継続）。`CUR_QUIZ/HINT_NAME/WIN_SRC` は可変。
- 主な関数：`startGame`→`pickRun`→`loadQuiz(0)` ／ `onWin`（クリア画面＋ボーナス＋NEXT）／ `runNext`（バー→次 or `allClear`）／ `allClear`。
...
## 11. 検証手段（ライブブラウザ無し環境向け）
- 構文：`node --check`。
- 動作：jsdom で例外なし＋ロジック検証（getBBox/getTotalLength はスタブ。カーニングはサンプル不可時フォールバック）。
- 見た目：Python `svgpathtools`＋`cairosvg` で SVG をラスタライズして確認。

## 12. 保留・未決
- **ALL CLEAR 画面のレイアウト**：仮（クリア5問の絵を横並び）。本デザインは後で作る。
- **図SVG**：現状 `step-pets` 以外は未作成。他クイズのヒント/勝利図・ALL CLEAR の絵は空表示（図はエディタで順次追加）。
- **手触りライブパネル**（実行中にCONFIGをスライダー調整）：未実装。次の山。レイアウト編集とは別物（静的 vs ライブ）。
